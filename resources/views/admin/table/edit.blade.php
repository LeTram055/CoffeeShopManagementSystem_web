...
<div class="row justify-content-center align-items-center">
    <div class="col-6 border rounded-3 p-5 custom-shadow mb-3">
        <h3 class="text-center title2 mb-4">Cập nhật bàn</h3>
        <form name="frmEdit" id="frmEdit" method="post" action="{{ route('admin.table.update') }}">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control rounded-2" id="table_number" name="table_number"
                    value="{{ old('table_number', $table->table_number) }}" placeholder="Nhập số bàn">
                @error('table_number')
            ...
            <div class="form-group mb-3">
                ...
                <input type="number" step="1" class="form-control rounded-2" id="capacity" name="capacity"
                    value="{{ old('capacity', $table->capacity) }}" placeholder="Nhập số lượng ghế">
                @error('capacity')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control rounded-2" id="status_id" name="status_id">
                    ...
                    <option value="{{ $status->status_id }}"
                        {{ old('status_id', $table->status_id) == $status->status_id ? 'selected' : '' }}>
                        {{ $status->name }}
                    </option>
                    @endforeach
                </select>
