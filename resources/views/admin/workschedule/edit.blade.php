...
<div class="row justify-content-center align-items-center">
    <div class="col-6 border rounded-3 p-5 custom-shadow">
        <h3 class="text-center title2 mb-4">Cập nhật lịch làm việc</h3>
        <form name="frmEdit" id="frmEdit" method="post" action="{{ route('admin.workschedule.update') }}">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control rounded-2" id="shift_id" name="shift_id">
                    ...
                    <option value="{{ $shift->shift_id }}"
                        {{ old('shift_id', $schedule->shift_id) == $shift->shift_id ? 'selected' : '' }}>
            ...
            <div class="form-group mb-3">
                ...
                <input type="date" class="form-control rounded-2" id="work_date" name="work_date"
                    value="{{ old('work_date', $schedule->work_date ? $schedule->work_date->format('Y-m-d') : '') }}">
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control rounded-2" id="status" name="status">
                    <option value="scheduled" {{ old('status', $schedule->status) == 'scheduled' ? 'selected' : '' }}>Đã
                        lên lịch</option>
                    <option value="completed" {{ old('status', $schedule->status) == 'completed' ? 'selected' : '' }}>
                        Hoàn thành</option>
                    <option value="absent" {{ old('status', $schedule->status) == 'absent' ? 'selected' : '' }}>Vắng mặt
                    </option>
                </select>
                @error('status')
                <small class="form-text text-danger">{{ $message }}</small>
            ...
            <div class="form-group mb-3">
                ...
                <input type="number" step="0.1" class="form-control rounded-2" id="work_hours" name="work_hours"
                    value="{{ old('work_hours', $schedule->work_hours) }}" placeholder="Nhập số giờ làm">
                @error('work_hours')
                <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
